...
final class GeoRisksStore {
    ...
    //TODO: Verify is this struct is being used
    func onRisksButtonTapped(_ risque: Risk) {
        self.risksState.selectedRisque = .init(risque: risque)
    }
    ...
    private func reloadRisks() async {
        do {
            ...
            self.risksState.risquesError = nil
            let (risques, community) = try await risksClient.risques(at: location)
            self.risksState.risks = risques
            self.risksState.risquesDescription = community ?? "(\(location.latitude), \(location.longitude))"
        } catch {
            // exit application if cast fails
            self.risksState.risquesError = (error as! RisksClientError)
        }
    }
